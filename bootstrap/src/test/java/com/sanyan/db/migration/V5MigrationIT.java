 ...
 *
 * <p>每个 @Test 在 {@link #runMigrationsUpToV5()} 里先 {@code Flyway.clean()} 再
 * migrate，保证用例之间互相隔离，不依赖执行顺序。
 */
class V5MigrationIT extends PostgresTestcontainerSupport {
    ...
    private static void runMigrationsUpToV5() {
    ...
    private static Map<String, ColumnSpec> describeColumns(String tableName) throws Exception {
        ...
        try (Connection conn = newConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    // BIGSERIAL 是 PG 的伪类型：底层 udt_name 仍是 int8，但若列默认值来自
                    // sequence，pg_get_serial_sequence 会非空。这里用 pg_get_serial_sequence 区分。
    ...
    private record ColumnSpec(String typeName, boolean nullable) {}
}
