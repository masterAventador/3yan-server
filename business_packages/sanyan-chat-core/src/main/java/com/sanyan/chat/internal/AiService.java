...
public class AiService {
    ...
    public String assembleSystemPrompt(String characterPrompt, String time) {
        // [时间感知] 段配合 PromptBuilder 在跨时间相邻消息间插入的 system role
        // "（时间跳跃：xxx）" 标记使用：LLM 看到 system 标记知道对话间隔但不会
        // 复读到回复（之前用 message content prefix 的方案被豆包直接复读到回复里）。
        return characterPrompt
                + "\n\n[当前时间] " + time
                + "\n\n[时间感知] 对话历史里如果出现 system role 的"
                + "\"（时间跳跃：...）\"标记，表示该处对话发生了较长时间间隔。"
                + "回复时如需提及过去对话，请结合当前时间和这些标记用自然时间词"
                + "（如\"刚才\"、\"今天上午\"、\"昨天\"、\"前天\"、\"上周\"），"
                + "不要默认所有历史都是刚发生的。"
                + "时间跳跃标记本身是系统元数据，不要复述到回复中。";
    }
