 ...
 * 消息持久化领域事件。
 *
 * <p>S3 Phase 5 起从 {@code sanyan-business} 上提到 chat-api（包名 {@code com.sanyan.chat.event}
 * 保持不变）。{@code MessageService}（chat-core internal）在消息成功保存后通过
 ...
 * <p>当前订阅方：
 * <ul>
 *   <li>{@code SummaryScheduler}（Plan 2 N3）：累积 30 条新消息触发摘要</li>
 *   <li>{@code UserMessageProfileRefreshListener}（Plan 2.5）：每条用户消息触发画像刷新</li>
 *   <li>{@code MessageEmbeddingIndexListener}（Plan 2 P5）：累积 chunk 后送 RAG 索引队列</li>
 * </ul>
 *
 * <p>字段约定：
 * <ul>
 ...
 *   <li>{@code characterId}：消息所属角色 id（MVP 阶段固定为 1L = 小婉，预留多角色扩展）</li>
 *   <li>{@code role}：发送者角色，取值见 {@link com.sanyan.chat.SenderType}
