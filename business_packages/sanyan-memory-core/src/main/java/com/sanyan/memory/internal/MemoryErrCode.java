...
/**
 * Memory 模块错误码 enum（Plan 2 引入；Plan 2.5 重命名 PROFILE_MERGE_CONFLICT → PROFILE_REFRESH_CONFLICT）。
 *
 ...
 *
 * <ul>
 *   <li>{@link #PROFILE_REFRESH_CONFLICT}：自然语言画像刷新乐观锁冲突，重试 1 次仍失败时抛出。
 *       由调用方（{@code UserMessageProfileRefreshListener}）决定重新入队或丢弃；不在 service 层再吞掉。</li>
